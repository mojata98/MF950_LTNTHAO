 ...
 */
$('#txtEmployeePhoneNumber').blur(function () {
    let value = $(this).val();
    if (!formatPhoneNumber(value)) {
        // Thay đổi border của input sang trạng thái lỗi
        $('input[required]').removeClass('border-default');
        $(this).addClass('border-red');
        alert("Thông tin số điện thoại không hợp lệ!");
    }
    else {
        $(this).removeClass('border-red');
        $(this).css('background-color', '#ffffff');
 ...
 * Author: LNTHAO (23/07/2021)
 */
$('#txtEmployeeCMND').blur(function () {
    let value = $(this).val();
    if (!formatIdentityCode(value)) {
        // Thay đổi border của input sang trạng thái lỗi
        $('input[required]').removeClass('border-default');
        $(this).addClass('border-red');
        alert("Thông tin số CMND không hợp lệ!");
    }
    else {
        $(this).removeClass('border-red');
        $(this).css('background-color', '#ffffff');
...
$('#btn-save').click(function () {
    if (($('#txtEmployeeCode').val() !== "") &&
        ($('#txtEmployeeName').val() !== "") &&
        ($('#dayOfJoin').val() !== "") &&
        formatEmail($('#txtEmployeeEmail').val()) &&
        formatIdentityCode($('#txtEmployeeCMND').val()) &&
        formatPhoneNumber($('#txtEmployeePhoneNumber').val())) {
        ...
        alert('Dữ liệu không hợp lệ. Không thể thêm mới.')
    }
